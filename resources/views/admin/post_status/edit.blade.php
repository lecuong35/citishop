...
@section('content')
    ...
    <div>
            ...
            <form action="" method="post" enctype="multipart/form-data" id="postStatus{{$postStatus['id']}}">
                @csrf 
                <div>
                    <label for="kindName">Name</label>
                    <input type="text" id="kindName" name="status" value="<?php echo $postStatus->status ?>" required
                ...
                disabled
                id="updateKind"
                onclick="postURL('/admin/post-status/{{$postStatus['id']}}', 'postStatus{{$postStatus['id']}}')">
                    Update
                </button> 
            </form>
