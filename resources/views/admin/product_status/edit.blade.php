...
@section('content')
    ...
    <div>
            ...
            <form action="" method="post" enctype="multipart/form-data" id="productStatus{{$productStatus['id']}}">
                @csrf 
                <div>
                    <label for="kindName">Name</label>
                    <input type="text" id="kindName" name="status" value="<?php echo $productStatus->status ?>" required
                ...
                disabled
                id="updateKind"
                onclick="postURL('/admin/product-status/{{$productStatus['id']}}', 'productStatus{{$productStatus['id']}}')">
                    Update
                </button> 
            </form>
